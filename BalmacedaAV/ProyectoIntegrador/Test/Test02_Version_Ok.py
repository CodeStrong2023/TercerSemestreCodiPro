...
def main():
    ...
    button = tk.Button(root, text="ACEPTAR", command=root.destroy, font=("Candara", 12, "bold"), bg="#87BEF6",
                       fg="white", width=45, height=3)
    button.pack(pady=20)
...
def elegir_codigo_postal(matriz):
    ...
    messagebox.showinfo("Información",
                        "A continuación verá las opciones de entrega a domicilio que tenemos para ofrecer dentro de "
                        "Mendoza")

    while not postal:
        codigo_str = simpledialog.askstring("Código Postal",
        ...
        if nuevo_costo >= 0:
            ...
            if not respuesta:
                messagebox.showinfo("Información",
                                    "Lo invitamos a visitar la sucursal de forma presencial y ser atendido por "
                                    "nuestro personal.")
                break

    if postal:
        while True:
            direc = simpledialog.askstring("Dirección",
                                           "Digite su calle, dirección y el envío llega en las primeras 24 horas "
                                           "desde que usted realiza el pago")
        ...
        direc = simpledialog.askstring("Dirección",
                                       "Si ha ingresado mal la dirección, vuelva a ingresarla, de lo contrario "
                                       "presione ENTER")
...
def agregar_producto(matriz_productos):
    ...
    if codigop:
        while True:
            nombre = simpledialog.askstring("Datos",
                                            "A continuación ingrese sus DATOS para confirmar la compra del "
            ...
            if nombre:  # Verificar si se ingresó un nombre
                ...
                messagebox.showinfo("Muchas Gracias",
...
def validar_tarjeta_debito(costo_de_envio, precio_producto):
    ...
    while True:
        numero_tarjeta = simpledialog.askstring("Número de tarjeta",
    ...
    while True:
        codigo_seguridad = simpledialog.askstring("Código de seguridad",
                                                  "Ingrese el código de seguridad de su tarjeta (son los 3 dígitos "
                                                  "que se encuentran en la parte posterior de su tarjeta): ")
    ...
    while True:
        try:
            mes = int(
                simpledialog.askstring("Mes de vencimiento", "Ingrese el mes de vencimiento de su tarjeta (MM): "))
            anio = int(simpledialog.askstring("Año de vencimiento",
